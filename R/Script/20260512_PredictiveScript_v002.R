...
# Data files (Eurostat "linear" CSV exports) live in the project folder
# "Data - csv.". There are duplicate project/R folders in this workspace, so
# v002 locates the CSV folder from the script path instead of assuming the
...
find_data_dir <- function(start_dir) {
  ...
  repeat {
    candidates <- file.path(current_dir, candidate_names)
  ...
  search_root <- dirname(normalizePath(start_dir, mustWork = FALSE))
  matches <- list.dirs(search_root, recursive = TRUE, full.names = TRUE)
  matches <- matches[basename(matches) %in% candidate_names]

  if (length(matches) == 1) return(matches)
  if (length(matches) == 0) stop("Kunne ikke finde mappen 'Data - csv'.")

  stop(
    "Fandt flere mulige 'Data - csv'-mapper:\n",
