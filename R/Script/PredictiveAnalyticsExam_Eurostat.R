...
library(httr)        # Eurostat API requests
...
hent_eurostat <- function(url) {
  svar <- GET(url)
  if (http_error(svar)) stop("API-fejl: ", status_code(svar))

  data      <- fromJSON(content(svar, as = "text", encoding = "UTF-8"))
  ...
  matrix_vals <- matrix(NA_real_, nrow = n_geo, ncol = n_tider,
  ...
  as.data.frame(matrix_vals) |>
    ...
    pivot_longer(-geo, names_to = "periode", values_to = "vaerdi") |>
    mutate(dato = ym(periode)) |>
    select(geo, dato, vaerdi) |>
    arrange(geo, dato)
}

# --- 1.1 Inflation: HICP annual rate of change (YoY %) ---
# Eurostat publishes the HICP annual rate of change across two datasets:
#   prc_hicp_manr: the main historical series (up to end of 2025)
#   prc_hicp_minr: the more recent series (2026 onwards)
# Both are fetched with unit=RCH_A (annual rate of change) to ensure a
# consistent measure throughout. We concatenate and remove any overlap with
# distinct(), giving a single unbroken series from 1997 to the latest month.
...
inflation <- bind_rows(inflation_gammel, inflation_ny) |>
  distinct(geo, dato, .keep_all = TRUE) |>
  arrange(geo, dato)
